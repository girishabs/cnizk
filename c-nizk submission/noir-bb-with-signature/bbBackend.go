...
func RunProver(job ProveJob) ProveResult {

	err := WriteProverToml(job)
	if err != nil {
		return ProveResult{Err: err}
	}

	workerDir, circuitDir := CreateTmpDir()
	...
	proof, _ := os.ReadFile(filepath.Join(outDir, "proof"))
	vk, _ := os.ReadFile(filepath.Join(outDir, "vk"))
	pub, _ := os.ReadFile(filepath.Join(outDir, "public_inputs"))
