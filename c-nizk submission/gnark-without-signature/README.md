...
- The system includes time-based expiration (TTL) for proofs
...
### 7. Server (`server/`)
Network server components for communication between prover and verifier.
...
### Security Features
- **Authenticity**: Data and constraints are signed by an authority
- **Privacy**: Only private data is hidden from the verifier
- **TTL Enforcement**: Proofs expire after a specified time
- **Non-interactive**: Proofs can be verified anywhere without interaction

## How It Works

1. **Setup Phase**:
   - An authority creates an asset with sensitive data
   - The authority defines constraints that the data must satisfy
   - The authority signs the data and constraints

2. **Proof Generation**:
   - The prover extracts data and converts it to field elements
   - Constraints are processed and converted to field elements
   - A MiMC hash is computed over the data and constraints
   - The authority signs the hash
   - A Groth16 proof is generated using the circuit

3. **Verification**:
   - The verifier receives the proof and verification key
   - The verifier checks the EDDSA signature
   - The verifier validates the circuit constraints
   - The verifier ensures the proof hasn't expired
...
This will start:
- Prover server on port 8080
- Verifier server on port 8082
- Prover-Verifier server on port 8084
...
- **Authenticity**: Signatures ensure data integrity and origin
- **TTL Protection**: Time-based expiration prevents stale proofs
- **Non-interactive**: Proofs can be verified anywhere

### Known Limitations
- **String Hashing**: Uses FNV (non-cryptographic) - can be improved with MiMC/SHA256
...
- **Performance**: EDDSA verification is computationally expensive

## Limitations

1. **Fixed Data Structure**: Currently limited to 5 predefined asset attributes
2. **String Hashing**: Uses FNV which is not cryptographically secure
3. **Constraint Visibility**: Constraint structure is public
4. **Performance**: Large numbers of constraints may exceed memory limits
5. **Static Circuit**: Requires recompilation for new layouts

## Future Improvements

1. **Enhanced String Hashing**: Replace FNV with cryptographic hash functions
2. **Dynamic Constraints**: Support for variable number of constraints
3. **Improved Performance**: Optimizations for EDDSA verification
4. **Better Error Handling**: More robust error handling and logging
5. **Extended Operations**: Additional constraint types and operations
6. **Multi-asset Support**: Support for multiple assets in a single proof
7. **Web API Interface**: RESTful API for easier integration
